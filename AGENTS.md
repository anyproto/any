...
3. **`nav` virtual built-in + tree UI** — every new object is auto-stamped
   ...
   `<select>` + popup form.
4. **Frontend shell modularity + performant spaces rail** — the Vite app keeps
   a minimum shell core in `web/app/src/app/`, `components/layout/`,
   `components/ui/`, and `shared/`. Pane 1 has persisted expanded widths plus a
   separate persisted fully-closed mode (`any.layout.spacesRailClosed.v1`).
   Closing the rail animates the pane to width 0. The spaces list virtualizes
   large lists and uses memoized rows with row-specific Jotai selectors. Pane 2,
   the visible current-space/object sidebar, also has a persisted fully-closed
   mode (`any.layout.spaceContentsClosed.v1`). Both sidebars are toggled from
   two Anytype-style buttons in the object-view header next to Back/Forward.
4. **Frontend app shell + modules** — `web/app` is the Vite frontend.
   Keep the core small (`main.tsx`, `App.tsx`, shell, `shared/`,
   `components/ui/`) and add product behavior through feature modules
   and registries such as `src/app/viewModules.tsx`. Persisted Jotai
   atoms must use `src/shared/storage.ts` so Vitest and browser
   fallback storage stay compatible.

**Always read the relevant `docs/NN-*.md` before writing code for an area**, and if
implementation diverges from a doc, update the doc in the same change. For frontend
work, also read `docs/agents/README.md` and the active `docs/specs/PR-*.md`.
