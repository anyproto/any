...
func TestBeforeModify_Reaction_OwnSlot(t *testing.T) {
	...
	if op.Payload == nil || op.Payload.Type() != anyenc.TypeDateTime {
		t.Fatalf("react own slot: payload not rewritten to an instant, got %v", op.Payload)
	}
	ms, err := op.Payload.DateTimeMillis()
	if err != nil {
		t.Fatalf("react own slot: payload not a datetime: %v", err)
	}
	if got := ms / 1000; got != 1700000050 {
