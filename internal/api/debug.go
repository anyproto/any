...
type LocalDiscoveryRequest struct {
	...
	// them stopped; true starts them at once. Held for the process
	// lifetime, across logout and account switches; a host restates it
	// after a restart, and may state it before the first POST /v1/auth.
	Enabled bool `json:"enabled"`
}
