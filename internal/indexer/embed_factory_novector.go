...
// llama.cpp purego/ffi bindings out of builds that don't want them
// (notably gomobile/Android, whose missing libffi otherwise panics at
// startup). See docs/13-index.md § build tags and the
// `vector && !gomobile && !mobile` variant in embed_factory_vector.go.
