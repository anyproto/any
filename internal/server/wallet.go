...
// wallet. On first generation the returned firstRun flag is true, and
// the caller is expected to display Mnemonic() once.
func OpenWallet(path, passkey, mnemonic string) (provider *auth.FileProvider, firstRun bool, err error) {
	p, err := auth.NewFileProvider(auth.FileProviderConfig{Path: path, Passkey: passkey, Mnemonic: mnemonic})
	if err != nil {
