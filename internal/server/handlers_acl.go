...
//	@Description	Withdraws this account's pending join request, from any device of the account. The space row flips to status "deleted" account-wide; POST /v1/spaces/join with a valid invite re-requests. A request already gone from the chain with no membership behind it (declined, or withdrawn elsewhere) is settled by the call: 204, row "deleted". 409 space.join_not_pending when the row is not joining, or when the owner accepted first — the row then settles to active on its own.
//	@Tags			acl
//	@Param			spaceId	path	string	true	"Space ID"
//	@Success		204
