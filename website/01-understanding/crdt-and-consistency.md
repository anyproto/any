...
|----------|-----------------------------------------------|-----|
| A write can fail because of another writer | Yes — serialization failure, OCC version mismatch, unique violation | No. Writes always commit locally. Only *validation* (schema, handler rules, a storage collection the object's type does not declare, tombstone) rejects. |
