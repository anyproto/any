...
`make build` adds the local embedder (`-tags llamacpp`) and its llama.cpp libraries; a bare `go build` serves full-text search, and vector search only through an online provider named by `index.openai.*`. `go test ./...` runs everything except the local-embedder tests, which `make test` adds. On NixOS run builds and the binary through `nix develop -c …` so the local embedder finds `libffi` ([Builds and CI](../operations/builds-and-ci.html)).
