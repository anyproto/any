...
> **Note.** The frame set and `closed` reasons are identical to every other windowed subscribe. Recover from `overflow` / `drifted` / `server_shutdown` / `sdk_closed` / `deauthorized` the same way: reopen the POST and take the fresh snapshot.
