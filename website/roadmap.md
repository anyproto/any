...
| Data plane | objects, types and properties with formats and select options, Mongo-style query with windowed subscribe over SSE, aggregation pipelines, version history, runtime dataset schemas with idempotent upsert, datetime instants, derived `createdAt`/`modifiedAt`/`author` — [Database](database/index.html) |
...
- **Windows.** Server and CLI have nothing Unix-specific left; the single-instance PID lock needs a Windows-friendly replacement and a verification pass.
