...
Every index document is `{id, scope, objectId, dataset, recordId, chunk?, data, title, hash, applySeq, vector?, pending?}` in a collection per space, and its id is
...
That shape is what makes removal cheap: deleting an object is a prefix delete on `objectId:`, detaching a type is a prefix delete on `objectId:<dataset>:`, and deleting one record is a range delete over its chunks. The database lives at `<account-dir>/index/index.db` and is entirely derived state.

## Chunkers, scopes, gating

| Source | Dataset in hits | Gate | Scope | Document |
|---|---|---|---|---|
| chat | `chat_messages` | a type owning the collection is attached | `chat` | one message, `text` only (no author, reactions, attachments) |
| editor | each editor collection — `editor_blocks` or `<typeId>_<key>` | per collection: a type owning it is attached | `basic` | a **window** of consecutive blocks (~1.5 KB, broken before each heading), `recordId = win_<firstBlockId>` |
...
| runtime datasets | the dataset's collection | per dataset: its type is attached | `basic` (default) | title line + text fields from the declared `x-search` mapping |

- **Gating.** Module and runtime collections are indexed only while the object carries a type that owns them — the `owners` a collection lists in `GET …/datasets`. Detach the last owning type and the object's documents for that collection are evicted.
- **Editor windows.** One document per block would give ~100-character chunks that embed badly and skew BM25 length normalization; windows avoid both. Because a window spans several records, the editor chunker rebuilds the object's full window set per editor collection and the indexer diffs it against stored content hashes — only new or changed windows are re-embedded, and an edit in a part's own editor never touches the shared body's documents.
...
| a type detached | prefix delete `objectId:dataset:` for each collection it was the last owner of |
