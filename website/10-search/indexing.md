...
- **Gating.** Module and runtime storage collections are indexed only while the object's type owns them — the `owners` a storage collection lists in `GET …/datasets` — or while the object is the definition root that declares that owner and hosts its own records, as the general chat does. Retype the object away from the owner and its documents for that storage collection are evicted.
...
- **Property values** index by default. A property definition's `meta.index` is a three-state override: absent means scope `props`, `"<scope>"` routes it there (and into the vector pipeline), `"none"` excludes it — the opt-out for blobs and noisy enums. String, array (newline-joined) and number values index; booleans, null and objects never do. Type and collection definition rows are skipped wholesale.
