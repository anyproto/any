...
The records live in a collection **namespaced to the type**, `<typeId>_<key>` — the `collection` every declaration reply and listing carries, and the `dataset` value on every read and write. Two types can each declare `entries` without colliding. Registered built-in types (`dataview`, `nav`, `page`, …) refuse runtime definitions with `400 type.registered`. Runtime datasets are for *your* types.
