...
A runtime dataset is a collection you define on one of your own types, at runtime, with a declarative schema. The schema syncs like any other data, and every peer enforces it on apply — required fields, who may edit a field after creation, who may delete a record, server-derived creator and time stamps, and whether record ids are auto-derived or caller-supplied.

Built-in types (`chat`, `editor`, …) have statically declared datasets and refuse runtime definitions with `400 type.registered`. Runtime datasets are for *your* types.
...
```sh
curl -X POST http://127.0.0.1:7001/v1/spaces/$SPACE/types/$TYPE/datasets \
...
any type dataset add $SPACE $TYPE --draft @articles.json
```

| Field | Meaning |
|---|---|
| `name` | Collection name, pinned and space-unique. Conflicts with built-ins, handler datasets or other definitions → `409 dataset.name_conflict`; `prop` / `schema` are reserved → `400 request.invalid_field`. |
...
Behavioral parts are pinned for the definition's life: `name`, `dynamic`, `idRule`/`idPattern`/`idMaxLen`, `deleteBy`, `skipHistory`, and every field's `key`/`kind`/`shape`/`scope`/`required`/`mutableBy`/`stamp`. To change one, remove the definition and add a new one.
...
```json
{ "datasets": [ { "id": "…", "name": "articles", "displayName": "Articles",
    "idRule": "user", "deleteBy": "author",
    "search": { "title": "title", "text": "body" },
...
A definition whose folded declaration fails validation is listed with `invalid: true` and `invalidReason` — it never registers or accepts data, but stays visible so it can be repaired or removed. Definitions racing in from other members resolve deterministically: the smallest definition id wins; built-in names always win; the loser stays listed but never registers.

Runtime datasets also appear in the space's discovery document, `GET /v1/spaces/:spaceId/datasets`, as JSON Schema with `typeId` and the behavioral keywords `x-mutable-by`, `x-stamp`, `x-delete-by`, `x-id` / `x-id-pattern` / `x-id-max-length`, `x-search`, plus the standard `required` list. `any datasets $SPACE` prints the same.
...
curl -X POST http://127.0.0.1:7001/v1/spaces/$SPACE/query \
  -d '{"objectId": "'$OBJ'", "dataset": "articles", "sort": ["-createdAt"]}'
```
...
> **Note.** Records live on objects carrying the owning type, so the first write to a fresh object fails until the type is attached. Stamped fields (`creator`, `createTime`, `modifyTime`) are instants and identities the server fills in; a payload that sets them is rejected.
