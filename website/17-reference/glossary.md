...
| **Dataset** | A named collection of records that lives on an object — `chat_messages`, `editor_blocks`, or a runtime-defined one — read through `/query` and written through `/modify` or its module's routes. — [Runtime datasets](../database/runtime-datasets.html) |
...
| **Usecase** | A set of well-known `system:` bundles from the server's embedded catalog, plus the usecases it requires; one setup call installs or adopts them all and returns the type and property ids to write with. — [Bundles](../collaboration/bundles.html#the-usecase-catalog) |
| **Type** | A named schema attached to objects (`any.types`), owning property definitions and parts; resolved by `xKey`, stored by id. — [Types and properties](../database/types-and-properties.html) |
| **Part** | A display unit of a type — a body, a transcript, a task list — owning the datasets a module serves. — [Modules](../types/index.html) |
| **Module** | The server code that serves a part's dataset: `records` (the generic schema handler), `editor` or `chat`; a shared dataset is the module's canonical collection, a namespaced one is `<typeId>_<key>`. — [Modules](../types/index.html) |
| **xKey** | A type's stable programmatic handle (`book`, `wiki`), unique per space, unaffected by display-name renames; a property's xKey is its handle within the type. — [Types and properties](../database/types-and-properties.html) |
...
| **Local store** | Device-local collections at `/v1/local` with the same query and aggregation power but no sync, no CRDT and no subscribe. — [HTTP API](http-api.html) |
