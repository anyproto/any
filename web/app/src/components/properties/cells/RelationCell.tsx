      ...
      <PopoverAnchor asChild>
        <div className="relative flex h-full w-full items-center">
          <button
            ...
            className={cn(
              'flex h-full min-w-0 flex-1 items-center gap-1 truncate px-2 py-1 text-left',
          ...
          >
            {!value ? (
              ...
              <RelationChip name={nameOf(linked) ?? 'Untitled'} />
            ) : (
          ...
          {linked && (
            <button
              type="button"
              aria-label={`Open ${nameOf(linked) ?? 'Untitled'}`}
              onClick={(e) => {
                e.stopPropagation();
                setActiveView({ kind: 'object', objectId: linked.id });
              }}
              className={cn(
                'mr-1 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded text-foreground/40',
...
function RelationChip({ name }: { name: string }) {
  return (
