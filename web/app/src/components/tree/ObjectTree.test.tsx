...
import { buildBatchFolderMoves, ObjectTree } from './ObjectTree';
...
describe('<ObjectTree>', () => {
  ...
  it('builds stable batch move operations into a folder', () => {
    const qc = new QueryClient();
    ...
    qc.setQueryData(['objects', 'spc-test', 'children', 'obj-folder'], [
      CHILD_ITEM,
    ]);

    const moves = buildBatchFolderMoves(
      qc,
      'spc-test',
      ['obj-item-2', 'obj-item-1'],
      'obj-folder',
    );

    expect(moves.map((m) => m.objectId)).toEqual(['obj-item-1', 'obj-item-2']);
    expect(moves).toEqual([
      {
        objectId: 'obj-item-1',
        fromParentId: '',
        toParentId: 'obj-folder',
    ...
    expect(moves[0]!.pos < moves[1]!.pos).toBe(true);
  });
});
