...
interface Props {
  ...
  onCreated?: (typeId: string) => void | Promise<void>;
  quickCreate?: boolean;
}
...
export function CreateTypeDialog({
  ...
  const createType = useCreateType(spaceId);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <Step state={state} dispatch={dispatch} quickCreate={quickCreate} />
      </DialogContent>
    </Dialog>
  );
}
  ...
  if (state.kind === 'name') {
    return (
      <>
        ...
        <Footer state={state} dispatch={dispatch} quickCreate={quickCreate} />
      </>
    );
  }
      ...
      {state.kind === 'name' && (
        <>
          ...
          <Button
            ...
            onClick={() =>
              dispatch({ type: quickCreate ? 'start_create' : 'next_to_properties' })
            }
          >
            {quickCreate ? 'Create' : 'Next'}
          </Button>
        </>
      )}
