...
export function useSpace(id: string | null) {
  ...
  const cachedSpace =
    id != null ? qc.getQueryData<SpaceInfo[]>(spaceKeys.all)?.find((space) => space.id === id) : undefined;
  const cachedSpaceUpdatedAt =
    id != null && cachedSpace ? qc.getQueryState(spaceKeys.all)?.dataUpdatedAt : undefined;

  return useQuery({
    queryKey: id ? spaceKeys.one(id) : (['spaces', '__none__'] as const),
    queryFn: ({ signal }) => getSpace(id!, signal),
    enabled: id != null,
    ...(cachedSpace ? { initialData: cachedSpace } : {}),
    ...(cachedSpaceUpdatedAt != null ? { initialDataUpdatedAt: cachedSpaceUpdatedAt } : {}),
  });
}
...
export function useCreateSpace() {
  ...
  return useMutation({
    ...
    onSuccess: (created) => {
      // Make the new space visible synchronously. AppShell validates
      // activeSpaceId against this list; if we only invalidate, it can
      // briefly see the old list and snap selection back before the
      // refetch returns.
      qc.setQueryData<SpaceInfo[]>(spaceKeys.all, (spaces) => {
        const current = spaces ?? [];
        const withoutDuplicate = current.filter((space) => space.id !== created.id);
        return [...withoutDuplicate, created].filter((space) => space.status !== 'deleted');
      });
      ...
      qc.setQueryData(spaceKeys.one(created.id), created);
    },
  });
}
...
export function useDeleteSpace() {
  ...
  return useMutation({
    ...
    onSuccess: (_void, id) => {
      void qc.invalidateQueries({ queryKey: spaceKeys.all });
      qc.removeQueries({ queryKey: spaceKeys.one(id) });
    },
  });
}
