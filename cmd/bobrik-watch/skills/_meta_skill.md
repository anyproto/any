...
**System skills** (deploy-pipeline-managed — `_any`, `_toolcaller`, `_soul`, `_space_context`, `_meta_skill`) are named with a leading underscore and are excluded from the list above. They shape the system prompt itself and get overwritten on every deploy. Don't `_`-prefix your own skills, and they'll show up in the list.
