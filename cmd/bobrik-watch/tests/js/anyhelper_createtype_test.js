...
export function main(args) {
  ...
  h.check("read role", getProp(obj, typeName + ".role") === "_main", getProp(obj, typeName + ".role"));
  h.check("read count", getProp(obj, typeName + ".count") === 3, "" + getProp(obj, typeName + ".count"));
  var hist = getProp(obj, typeName + ".history");
  ...
  up = c.updateObject(co.id, { properties: dottedOne(typeName + ".history", ["bafyA", "bafyB", "bafyC"]) });
  h.check("update history ok", up && up.ok, JSON.stringify(up));
  var hist2 = getProp(c.getObject(co.id), typeName + ".history");
