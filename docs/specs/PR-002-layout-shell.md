...
- ~80 px wide. Resizable 60–200; default 80.
- Vertical strip of space "icons" — colored circles with an emoji or
  letter. Active space gets an accent ring.
- Bottom: a + button (creates a space — disabled in v1.PR#3 wires it),
  a settings cog (no action yet).
...
- **Persistence**: pane widths persisted under `any.layout.widths` via
  Jotai `atomWithStorage`. The storage key is bumped if the panel
  count or order changes.
...
2. Both dividers drag to resize. Min/max enforced (cursor changes;
   panel won't shrink past min).
3. Reloading restores the last divider positions.
...
- **Settings cog placement.** Pane 1 bottom feels right; alternative
  is the top-right of pane 2's header. Default: pane 1 bottom.
