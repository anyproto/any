...
- **Editing** types or properties (no API for that yet —
  `Types.UpdatePropertyMeta` returns `501`).
- **Deleting** types or properties (also `501`).
- **Editing property values on objects** — that's a property panel
  in pane 3, separate PR.
- **Custom icons** for types in the menu.
...
`useCreateObject` extended to pass `types: [typeId]` in the request
body so the SDK stamps the user type alongside `nav`.

### Cache + invalidation

- `useTypes(spaceId)` — `queryKey: ['types', spaceId]`.
- `useCreateType` invalidates that key on success.
...
7. After success the new type appears in the + New dropdown as
   `New {TypeName}`.
8. Selecting `New {TypeName}` POSTs /objects with `{types: [typeId]}`,
   the new object lands in the tree, gets selected, opens in pane 3.
