...
### Editor: BlockNote with the Mantine component adapter

The editor uses `@blocknote/core` and `@blocknote/react` for the editor
model and hooks, plus `@blocknote/mantine` for BlockNote's concrete
component adapter. This adapter is what renders the slash menu,
formatting toolbar, side menu, and block drag handles. Without it,
`BlockNoteViewRaw` renders editable content but disables the default
UI because no `ComponentsContext` is present.
