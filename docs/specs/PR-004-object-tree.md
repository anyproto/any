...
| `POST` | `/v1/spaces/:s/objects` | Create. Empty body `{}` is accepted; the server stamps the `nav` namespace defaults. Response: `{objectId}`. |
...
- **+ button placement.** The header gets `+ New` (single button,
  not a dropdown). Clicking it POSTs `/objects` with `{}` and selects
  the new id. Toast on error.
...
- **Unit** — `objects.ts`: queryObjects POST shape, createObject POST `{}`, error envelope passes through.
- **Component** — `ObjectRow`:
