...
| Programs | **not indexed** — a harness-declared user type whose `program_source` runtime dataset has no `search` mapping (docstrings are the docs, anybao ADR-010); discovery is the agent's prompt inventory + `help()` | anybao `runtime/src/program_schema.rs` |
