...
- The `account`/`device` property scopes and the type-delete /
  type-prop-delete endpoints are server-side `501 sdk.not_implemented`
  (`docs/07-roadmap.md`) — nothing to wrap. `attach`/`detach` are live
  but host-side: the JS client binds types at object create.
