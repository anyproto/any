...
| Server data | `web/app/src/lib/api/<group>.ts` today; `web/app/src/features/<name>/queries.ts` when a module grows | TanStack Query hooks (`useFoo()`, `useCreateFoo()`). |
...
| Module entry | `web/app/src/app/viewModules.tsx` or `web/app/src/features/<name>/index.ts` | Registration point for pane views or future plugin-style features. |
...
Keep the core small. `main.tsx`, `App.tsx`, `AppShell`, `shared/`, and
`components/ui/` should not learn product-specific rules about spaces,
objects, types, or editor behavior. Add those through feature modules
and registries.
...
Persisted atoms use `appStorage()` from `@/shared/storage`, not raw
`atomWithStorage(..., undefined, ...)`, so tests and constrained
browser contexts get a compatible fallback.
