...
type.not_found                   # 404 — unknown typeId on GET …/types/:typeId, GET …/types/:typeId/properties (existence-checked: a real type with no properties answers 200 [], an unknown id never does) and POST …/properties/:objectId/type/:typeId; 400 when a bundle ensure's rootType / rootProperties names a type the space does not have
membership.wrong_slot            # 400 — a write put a known collection id in any.type or a known type id in any.collections through a raw route (POST …/modify on the objects row, a bundle child, rootProperties keyed by a type); the membership routes and POST …/objects answer type.not_a_type / collection.not_a_collection instead
