...
device.pruned                    # 409 — this device was pruned: PUT /me is absorbed by the row's tombstone, activate is refused (without writing when the prune is already known); the peer id can never re-register (fresh `any init` to re-derive keys)
