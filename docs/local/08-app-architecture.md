...
`go build`, `go test`, and the Playwright command. The workflow opts
GitHub JavaScript actions into the Node 24 runtime while keeping the
app toolchain pinned separately. Playwright E2E runs only when
`docs/dev/stage1.yaml` is present in the checkout; clean/forked runs
skip that slice instead of booting the server against the missing
`../test-etc/staging.yml` fallback and failing with a misleading curl
connection error. Meaningful e2e spec files should be added as app
flows stabilize.
...
The table module has explicit extension points now:

- `TableView.tsx` is composition only. It wires the title, toolbar,
  renderer, and settings panel, but should not grow feature logic.
- `useTableViewController(typeId, rowHeight)` owns data loading,
  paging, client-side name filtering, sort state, virtualization, type
  title metadata, and row creation.
- `useTableProperties(spaceId, typeId)` owns type-property loading,
  user-property filtering, visibility, local order, column drag state,
  persisted widths, and resize normalization.
- `useDataViewLayout(typeId)` owns the current local layout selection.
  A future saved-view API should replace this atom behind the same
  hook instead of making renderers know storage details.
- Layout renderers (`TableRows`, `ListRowsView`, future board/gallery
  views) receive rows and visible properties from those hooks. They
  must not call object/type queries directly.
- Toolbar and settings routes live in `TableToolbar.tsx` and
  `TableSettingsPanel.tsx`; each nested settings screen lives under
  `components/tables/settings/`. Add new list options as a screen in
  that folder, then route to it from the panel.

This is the data-view contract for new view work: reuse the controller,
property model, and settings navigation; add only a renderer and any
small renderer-specific UI state. If a future view needs different
server data, add a new controller hook beside `useTableViewController`
rather than adding conditionals to `TableView.tsx`.
`Settings` opens a
